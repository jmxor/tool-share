...
export async function insertDirectMessage(
  ...
  try {
    ...
    await conn.query(insertQuery, [
      String(user1_id.rows[0].id),
      String(user2_id.rows[0].id),
      msg,
    ]);
    ...
    await sendNotificationEmail(
      user2_id.rows[0].id,
      "messages",
      `You have received a message from ${user1}.`
    );
  } catch {
    return null;
  }
...
export async function deleteConversationAction(conversationId: number) {
  try {
    ...
    await conn.query(deleteMessages, [
      result.rows[0].user1_id,
      result.rows[0].user2_id,
    ]);
    await conn.query(deleteConversation, [
      result.rows[0].user1_id,
      result.rows[0].user2_id,
    ]);
