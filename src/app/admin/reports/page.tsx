...
export default function ReportsManagement() {
  ...
  const [currentStatus, setCurrentStatus] = useState<ReportStatus | "all">(
    "all",
  );
  const [isLoading, setIsLoading] = useState(true);

  const fetchReports = useCallback(
    async (page: number, status: ReportStatus | "all" = currentStatus) => {
      setIsLoading(true);
      try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-2">
        <Select
          value={currentStatus}
          onValueChange={(value) =>
            handleStatusChange(value as ReportStatus | "all")
          }
        >
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="max-w-md">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Report filed by {report.accuser.username}{" "}
                                against {report.accused.username}
                              </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4 py-4">
                              ...
                              <div>
                                ...
                                <Badge
                                  className={`mt-1 ${getStatusBadgeVariant(report.status)}`}
                                >
                                  {report.status}
                                </Badge>
                              </div>
